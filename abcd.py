...
def waist_eq(wavelength, waist, z, z0, n):
    return waist*sqrt(1+(((z-z0)*wavelength)/(pi*(waist**2)*n))**2)

def inverseR(wavelength, waist, z0):
    return z0/(z0**2 + (pi*waist**2/wavelength)**2)

def q(wavelength, waist, z0, n):
    return 1/((-1j*wavelength)/(pi*n*(waist_eq(wavelength, waist, 0, z0, n)**2)) + inverseR(wavelength, waist, z0))

def q2(wavelength, waist, z0, n, matrix):
    return( (matrix[0,0]*q(wavelength, waist, z0, n) + matrix[0,1])/(matrix[1,0]*q(wavelength, waist, z0, n) + matrix[1,1]) )

def waist2(wavelength, waist, z0, n, matrix):
    return sqrt((-wavelength)/(pi*((1/q2(wavelength, waist, z0, n, matrix)).imag)))
...
def interface(n1, n2, r1):
    return np.array([[1,0],[((n1-n2)/(r1*n2)),n1/n2]])

def s_mirror(R):
    return np.array([[1,0],[-2/R,1]], dtype=object)

def prop(d, n=1):
    return np.array([[1,d/n],[0,1]], dtype=object)
...
def loopMat(ops, args):
    ...
    for operation, arg in zip(ops,args):
        matrix = matmul(operation.op(*arg), matrix)
    return matrix